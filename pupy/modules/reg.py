...
from pupylib.PupyOutput import Color, List, Table, Line, MultiPart, TruncateToTerm
...
class reg(PupyModule):
    ...
    def _format_by_one(self, record):
        ...
        if is_key is None:
            ...
            self.log(Line('Key:', Color(key, 'green')))
            return
        ...
        self.log(
            List([
                Line('Key:', Color(key, color)),
                Line('Name:', Color(name, color)),
                Line('Value:', Color(
