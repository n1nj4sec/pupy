...
from packages.all.smbmap import connect
from netaddr import *
...
class SMBSpider(PupyModule):
    ...
    def init_argparse(self):
        ...
        example += '>> run smbspider -u john -p password1 -d TEST --regex password.* pwd.* --content 192.168.0.1\n'
    ...
    def run(self, args):
        ...
        self.client.load_package("smbspider")

        for host in hosts:
            self.info("Connecting to the remote host: %s:%s" % (host, str(args.port)))
            smbspider = self.client.conn.modules["smbspider"].SMBSpider(host, args.domain, args.port, args.user, args.passwd, args.hash, args.content, args.regex, args.share, args.exclude_dirs, args.pattern, args.max_size)
