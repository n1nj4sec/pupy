...
for decl in decls:
    ...
    if len(items) == 3:
        ...
        print('#define %(name)s ((%(restype)s(*)%(argtypes)s)imports[%(index)d].proc)' % locals(), file=hfile)
    elif len(items) == 2:
        # exported data
        typ, name = map(string.strip, items)
        print('#define %(name)s (*(%(typ)s(*))imports[%(index)s].proc)' % locals(), file=hfile)
    else:
    ...
    if name == "Py_InitModule4":
        ...
        print('#  if defined (__x86_64__)', file=cfile)
